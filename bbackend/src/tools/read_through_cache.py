...
def _load_messages_from_api(
    ...
    try:
        ...
        return [
            Message(username=msg["username"], message=msg["message"], images=msg["images"])
...
class ReadThroughCache:
    ...
    def _is_range_contained(self, target_start: str, target_end: str, cache_start: str, cache_end: str) -> bool:
    ...
    def _calculate_missing_ranges(self, start_date: str, end_date: str, 
        ...
        if current_dt < end_dt:
            missing_ranges.append((
                self._format_iso_date(current_dt),
                self._format_iso_date(end_dt)
            ))
        
        # Normalize ranges to avoid duplicate fetches
        return self._normalize_ranges(missing_ranges)

    def _find_in_cache(self, channel_id: str, start_date: str, end_date: str) -> Optional[List[Message]]:
    ...
    def _filter_messages_for_date_range(self, messages: List[Message], start_date: str, end_date: str) -> List[Message]:
        """
    ...
    def load(self, channel_id: str, start_date: str, end_date: str) -> List[Message]:
        ...
        logger.debug(f"Loading messages for channel {channel_id} from {start_date} to {end_date}")
        
        # Check cache first for exact match
        cached_messages = self._find_in_cache(channel_id, start_date, end_date)
        if cached_messages is not None:
            logger.debug(f"Cache hit: returning {len(cached_messages)} messages from cache")
            return self._filter_messages_for_date_range(cached_messages, start_date, end_date)

        # Find overlapping date ranges in cache
        overlapping_ranges = self._find_overlapping_ranges(channel_id, start_date, end_date)
        ...
        if overlapping_ranges:
            ...
            if missing_ranges:
                logger.debug(f"Need to fetch {len(missing_ranges)} missing date ranges: {missing_ranges}")
                
                # Fetch missing ranges from API
                for missing_start, missing_end in missing_ranges:
                    if self.use_mock:
                        new_messages = _load_mock_messages(channel_id, missing_start, missing_end)
                    else:
                        new_messages = _load_messages_from_api(channel_id, missing_start, missing_end)
                    
                    logger.debug(f"Fetched {len(new_messages)} new messages for range {missing_start} to {missing_end}")
            ...
            result = self._filter_messages_for_date_range(merged_messages, start_date, end_date)
            logger.debug(f"Returning {len(result)} messages after merging and filtering")
            return result
        else:
